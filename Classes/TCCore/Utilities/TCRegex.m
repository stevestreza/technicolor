...
-(id)initWithPattern:(NSString *)pattern options:(int)options
{
	if(self=[super init])
	{
		...
		if(err)
		{
			...
			[NSException raise:@"TCRegexException"
...
{ return [[[TCRegex alloc] initWithPattern:pattern options:options] autorelease]; }

+(TCRegex *)regexWithPattern:(NSString *)pattern
{ return [[[TCRegex alloc] initWithPattern:pattern options:0] autorelease]; }
...
@implementation NSString (TCRegex)

-(BOOL)matchedByPattern:(NSString *)pattern options:(int)options
{
	TCRegex *re=[TCRegex regexWithPattern:pattern options:options|REG_NOSUB];
...
{
	TCRegex *re=[TCRegex regexWithPattern:pattern options:options];
